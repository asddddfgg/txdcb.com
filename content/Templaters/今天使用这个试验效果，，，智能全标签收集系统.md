<%*
// ==========================================
// 全站标签自动收集器 - 一键生成所有标签内容
// 最终版：修复所有问题
// ==========================================

// 🎨 配置区域（请确认修改）
const TARGET_FOLDER = "02 Notes";  // 目标文件夹名称
const COLLECTION_TITLE_PREFIX = "标签收集：";  // 标题前缀

// 🚫 排除的文件列表（自动排除所有收集文件）
const EXCLUDE_FILES = [
    "万能标签搜索器.md",
    "全站标签总览.md"
];

// 获取当前模板文件名（用于排除自身）
const currentTemplateName = app.workspace.activeLeaf?.view?.file?.name || "未知模板";

// 如果当前模板不在排除列表中，添加它
if (!EXCLUDE_FILES.includes(currentTemplateName)) {
    EXCLUDE_FILES.push(currentTemplateName);
}

// 🕒 记录开始时间
const startTime = new Date();

// 🔍 第一步：扫描全站所有标签（排除目标文件夹中的收集文件）
console.log("开始扫描全站标签...");

// 获取所有文件，排除目标文件夹中的收集文件和指定排除文件
const allFiles = app.vault.getMarkdownFiles()
    .filter(file => {
        // 排除指定文件
        if (EXCLUDE_FILES.includes(file.name)) {
            return false;
        }
        
        // 排除所有包含"的名言金句.md"的文件（避免查询已生成的收集文件）
        if (file.name.includes("的名言金句.md") && file.path.includes(TARGET_FOLDER)) {
            return false;
        }
        
        // 排除总览文件
        if (file.name === "全站标签总览.md" && file.path.includes(TARGET_FOLDER)) {
            return false;
        }
        
        return true;
    });

let allTags = new Set();
let tagFileMap = new Map(); // 标签 -> 文件映射
let totalFilesScanned = 0;
let tagSentencesMap = new Map(); // 标签 -> 句子集合

// 📖 扫描所有文件，提取标签
for (const file of allFiles) {
    totalFilesScanned++;
    const content = await app.vault.read(file);
    const lines = content.split('\n');
    
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i].trim();
        
        // 基础过滤
        if (!line || line.length < 10) continue;
        if (line.startsWith('#') || line.startsWith('---')) continue;
        
        // 使用正则表达式匹配所有标签（改进版）
        const tagMatches = line.match(/#[^\s#<>[\]{}()，,。！!？?;；:：'"`~@#$%^&*+=|\\/]+/g);
        
        if (tagMatches) {
            tagMatches.forEach(tag => {
                // 清理标签格式
                const cleanTag = tag.trim();
                
                // 进一步过滤无效标签
                if (cleanTag.length > 2 && 
                    !cleanTag.includes('扫描文件') && 
                    !cleanTag.includes('有效文件') && 
                    !cleanTag.includes('收集内容') &&
                    !cleanTag.includes('const ') &&
                    !cleanTag.includes('let ') &&
                    !cleanTag.includes('await ') &&
                    !cleanTag.includes('tR +=') &&
                    !cleanTag.includes('function') &&
                    !cleanTag.includes('async ') &&
                    !cleanTag.startsWith('# ') &&
                    !cleanTag.startsWith('#\n') &&
                    !cleanTag.match(/^#[0-9]/) && // 排除 #1, #2 等编号
                    !cleanTag.includes('SEARCH_TAG') &&
                    !cleanTag.includes('OUTPUT_FILE')) {
                    
                    allTags.add(cleanTag);
                    
                    // 记录标签所在的文件
                    if (!tagFileMap.has(cleanTag)) {
                        tagFileMap.set(cleanTag, new Set());
                    }
                    tagFileMap.get(cleanTag).add(file.basename);
                    
                    // 提取包含标签的句子
                    if (!tagSentencesMap.has(cleanTag)) {
                        tagSentencesMap.set(cleanTag, new Map());
                    }
                    
                    // 清理句子格式
                    let cleanSentence = line
                        .replace(/^[>-]\s*/, '')
                        .replace(/\s+/g, ' ')
                        .replace(/`/g, '')
                        .trim();
                    
                    // 确保是真正的文本内容
                    if (cleanSentence.length >= 10 && 
                        !cleanSentence.startsWith('//') &&
                        !cleanSentence.includes('扫描文件') &&
                        !cleanSentence.includes('有效文件') &&
                        !cleanSentence.includes('收集内容') &&
                        !cleanSentence.includes('const ') &&
                        !cleanSentence.includes('let ') &&
                        !cleanSentence.includes('await ') &&
                        !cleanSentence.includes('tR +=') &&
                        !cleanSentence.includes('collectionContent') &&
                        !cleanSentence.includes('SEARCH_TAG') &&
                        !cleanSentence.includes('OUTPUT_FILE') &&
                        !cleanSentence.includes('EXCLUDE_FILES') &&
                        !cleanSentence.includes('totalFilesScanned')) {
                        
                        // 去重存储
                        if (!tagSentencesMap.get(cleanTag).has(cleanSentence)) {
                            tagSentencesMap.get(cleanTag).set(cleanSentence, {
                                sourceFile: file.basename,
                                sourcePath: file.path
                            });
                        }
                    }
                }
            });
        }
    }
}

// 📊 第二步：准备生成收集文件
console.log(`发现 ${allTags.size} 个标签，开始生成收集文件到 ${TARGET_FOLDER} 文件夹...`);

let createdFiles = 0;
let updatedFiles = 0;
let failedFiles = 0;
const allTagsArray = Array.from(allTags).sort();

// 辅助函数：清理文件名中的非法字符
function sanitizeFileName(filename) {
    return filename.replace(/[\\/*"<>:|?]/g, '-');
}

// 辅助函数：创建文件夹路径
function createFilePath(fileName) {
    return `${TARGET_FOLDER}/${fileName}`;
}

// 辅助函数：安全获取或创建文件到指定文件夹
async function safeCreateFile(fileName, content) {
    const filePath = createFilePath(fileName);
    
    try {
        // 确保目标文件夹存在
        const folder = app.vault.getAbstractFileByPath(TARGET_FOLDER);
        if (!folder) {
            await app.vault.createFolder(TARGET_FOLDER);
        }
        
        const targetFile = app.vault.getAbstractFileByPath(filePath);
        if (targetFile) {
            await app.vault.modify(targetFile, content);
            return 'updated';
        } else {
            await app.vault.create(filePath, content);
            return 'created';
        }
    } catch (error) {
        console.error(`保存文件 ${filePath} 时出错:`, error);
        return 'failed';
    }
}

// 生成总览文件内容
let overviewContent = `# 全站标签总览\n\n`;
overviewContent += `生成时间：${new Date().toLocaleString()}  |  扫描文件：${totalFilesScanned}  |  发现标签：${allTags.size}\n\n`;
overviewContent += `---\n\n`;

// 标签统计表格
overviewContent += `## 标签统计\n\n`;
overviewContent += `| 标签 | 内容数量 | 涉及文件 | 查看收集 |\n`;
overviewContent += `|------|----------|----------|----------|\n`;

// 为每个标签生成文件
for (const tag of allTagsArray) {
    const sentences = tagSentencesMap.get(tag) || new Map();
    const filesForTag = tagFileMap.get(tag) || new Set();
    
    // 如果这个标签没有实际内容，跳过生成文件
    if (sentences.size === 0) {
        continue;
    }
    
    // 生成单个标签收集文件
    const tagName = tag.replace('#', '');
    const safeFileName = sanitizeFileName(`${tagName}的名言金句.md`);
    const fileTitle = `${COLLECTION_TITLE_PREFIX}${tagName}`;
    
    let tagContent = `# ${fileTitle}\n\n`;
    tagContent += `收集时间：${new Date().toLocaleString()}  |  标签：${tag}  |  涉及文件：${filesForTag.size}  |  收集内容：${sentences.size}\n\n`;
    tagContent += `---\n\n`;
    
    // 内容列表
    const sortedSentences = Array.from(sentences.entries())
        .sort(([sentenceA], [sentenceB]) => sentenceA.localeCompare(sentenceB));
    
    for (const [sentence, info] of sortedSentences) {
        tagContent += `${sentence}\n\n`;
        tagContent += `↳ 来源：[[${info.sourceFile}]]\n\n`;
        tagContent += `---\n\n`;
    }
    
    tagContent += `*自动生成 · 最后更新：${new Date().toLocaleString()} · 标签：${tag}*\n`;
    tagContent += `*返回 [[全站标签总览]]*`;
    
    // 保存文件到目标文件夹
    const result = await safeCreateFile(safeFileName, tagContent);
    
    if (result === 'created') {
        createdFiles++;
    } else if (result === 'updated') {
        updatedFiles++;
    } else {
        failedFiles++;
        console.log(`标签 "${tag}" 保存失败`);
    }
    
    // 添加到总览
    overviewContent += `| ${tag} | ${sentences.size} | ${filesForTag.size} | [[${tagName}的名言金句]] |\n`;
}

// 完成总览文件
overviewContent += `\n---\n\n`;
overviewContent += `## 统计信息\n`;
overviewContent += `- 总扫描文件数：${totalFilesScanned}\n`;
overviewContent += `- 发现的标签数：${allTagsArray.length}\n`;
overviewContent += `- 有效标签数（有内容）：${createdFiles + updatedFiles}\n`;
overviewContent += `- 新创建文件：${createdFiles}\n`;
overviewContent += `- 更新文件：${updatedFiles}\n`;
overviewContent += `- 失败文件：${failedFiles}\n`;
overviewContent += `- 运行耗时：${((new Date() - startTime) / 1000).toFixed(2)}秒\n\n`;

overviewContent += `## 使用说明\n\n`;
overviewContent += `1. 本文件由 **全站标签自动收集器** 生成\n`;
overviewContent += `2. 点击表格中的链接查看每个标签的具体内容\n`;
overviewContent += `3. 标签按字母顺序排序\n`;
overviewContent += `4. 只生成有实际内容的标签收集文件\n\n`;

overviewContent += `*最后更新：${new Date().toLocaleString()}*`;

// 保存总览文件到目标文件夹
const overviewResult = await safeCreateFile("全站标签总览.md", overviewContent);

// 🎊 结果显示
const runTime = ((new Date() - startTime) / 1000).toFixed(2);

tR += `# 🎯 全站标签自动收集完成！\n\n`;

tR += `## 📊 执行统计\n\n`;
tR += `- **扫描文件**：${totalFilesScanned} 个（已排除收集文件）\n`;
tR += `- **发现标签**：${allTagsArray.length} 个\n`;
tR += `- **有效标签**：${createdFiles + updatedFiles} 个（有实际内容）\n`;
tR += `- **新创文件**：${createdFiles} 个\n`;
tR += `- **更新文件**：${updatedFiles} 个\n`;
tR += `- **失败文件**：${failedFiles} 个\n`;
tR += `- **运行耗时**：${runTime} 秒\n`;
tR += `- **保存位置**：[[${TARGET_FOLDER}]] 文件夹\n\n`;

if (failedFiles > 0) {
    tR += `> ⚠️ **注意**：有 ${failedFiles} 个文件保存失败，可能是因为文件名包含特殊字符。\n\n`;
}

tR += `## 🏷️ 发现的标签（前20个）\n\n`;
const previewTags = allTagsArray.slice(0, 20);
previewTags.forEach(tag => {
    const sentences = tagSentencesMap.get(tag);
    const count = sentences ? sentences.size : 0;
    tR += `- ${tag} (${count}条内容)\n`;
});

if (allTagsArray.length > 20) {
    tR += `\n...等 ${allTagsArray.length} 个标签\n`;
}

tR += `\n## 📁 生成文件\n\n`;
tR += `1. **[[${TARGET_FOLDER}/全站标签总览]]** - 包含所有标签的索引表格\n`;
tR += `2. **标签收集文件** - 每个有效标签生成一个独立文件（共 ${createdFiles + updatedFiles} 个）\n`;
tR += `3. **存储位置**：所有文件已保存到 **${TARGET_FOLDER}** 文件夹\n\n`;

// 显示前3个有内容的标签
const previewTagsWithContent = allTagsArray.filter(tag => {
    const sentences = tagSentencesMap.get(tag);
    return sentences && sentences.size > 0;
}).slice(0, 3);

if (previewTagsWithContent.length > 0) {
    tR += `## 🔍 标签内容示例\n\n`;
    
    for (const tag of previewTagsWithContent) {
        const sentences = tagSentencesMap.get(tag);
        const previewSentences = Array.from(sentences.keys()).slice(0, 1);
        
        tR += `### ${tag}\n`;
        previewSentences.forEach(sentence => {
            tR += `> ${sentence}\n\n`;
        });
        const tagName = tag.replace('#', '');
        tR += `[[${TARGET_FOLDER}/${sanitizeFileName(`${tagName}的名言金句`)}]] | `;
    }
}

tR += `\n\n---\n\n`;
tR += `## 🚀 下一步操作\n\n`;
tR += `1. 查看 **[[${TARGET_FOLDER}/全站标签总览]]** 获取完整标签列表\n`;
tR += `2. 点击任意标签链接查看详细的名言金句\n`;
tR += `3. 所有文件已保存到：**${TARGET_FOLDER}** 文件夹\n`;
tR += `4. 如需重新生成，请再次运行此模板\n\n`;

tR += `> ✅ **智能排除**：模板已自动排除所有收集文件和模板自身，避免重复查询\n`;
tR += `> 📅 **最后运行**：${new Date().toLocaleString()}\n`;
%>